...
use oauth2_passkey::{
    AuthenticatorInfo, DbUser, O2P_ROUTE_PREFIX, SessionUser, get_authenticator_info_batch,
...
async fn user_summary(auth_user: AuthUser, user_id: Path<String>) -> impl IntoResponse {
    ...
    let user = match get_user(&session_user, &user_id).await {
        Ok(user) => user,
        Err(e) => {
...
mod tests {
    ...
    // and router creation. These provided no validation value.
}

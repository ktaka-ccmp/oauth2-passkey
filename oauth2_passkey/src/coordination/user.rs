...
mod tests {
    use super::*;
    use std::env;
    use serial_test::serial; // Assuming you'll add `serial_test` to dev-dependencies
    ...
    async fn setup_test_db() -> Result<(), Box<dyn std::error::Error>> {
        ...
        UserStore::init().await.map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
        PasskeyStore::init().await.map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
        OAuth2Store::init().await.map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    ...
    async fn test_update_user_account_success() {
        ...
        UserStore::upsert_user(initial_user.clone()).await.expect("Failed to insert initial user");
        ...
        assert!(result.is_ok(), "update_user_account failed: {:?}", result.err());
    ...
    async fn test_delete_user_account_success() {
        ...
        UserStore::upsert_user(test_user.clone()).await.expect("Failed to insert user");
        ...
        PasskeyStore::store_credential(cred1.credential_id.clone(), cred1.clone()).await.expect("Failed to store cred1");
        PasskeyStore::store_credential(cred2.credential_id.clone(), cred2.clone()).await.expect("Failed to store cred2");

        // 3. Create OAuth2 accounts
        let oauth_acc1 = create_test_oauth2_account("oauth-acc-1", user_id_to_delete, "google", "google-id-1");
        let oauth_acc2 = create_test_oauth2_account("oauth-acc-2", user_id_to_delete, "github", "github-id-1");
        OAuth2Store::upsert_oauth2_account(oauth_acc1).await.expect("Failed to upsert oauth_acc1");
        OAuth2Store::upsert_oauth2_account(oauth_acc2).await.expect("Failed to upsert oauth_acc2");

        // 4. Call the actual function
        let result = super::delete_user_account(user_id_to_delete).await;

        // 5. Verify returned credential IDs
        assert!(result.is_ok(), "delete_user_account failed: {:?}", result.err());
        let mut returned_credential_ids = result.unwrap();
        returned_credential_ids.sort(); // Sort for consistent comparison
        assert_eq!(returned_credential_ids, vec!["credential-1", "credential-2"]);

        // 6. Verify user is deleted
        let user_from_db = UserStore::get_user(user_id_to_delete).await.expect("DB error getting user");
        assert!(user_from_db.is_none(), "User was not deleted from DB");

        // 7. Verify passkeys are deleted
        let passkeys_from_db = PasskeyStore::get_credentials_by(CredentialSearchField::UserId(user_id_to_delete.to_string()))
            .await
        ...
        let oauth_accounts_from_db = OAuth2Store::get_oauth2_accounts_by(AccountSearchField::UserId(user_id_to_delete.to_string()))
            .await
            .expect("DB error getting oauth accounts");
        assert!(oauth_accounts_from_db.is_empty(), "OAuth2 accounts were not deleted");
    }
    ...
    async fn test_gen_new_user_id_success() {
        ...
        let user_from_db = UserStore::get_user(&generated_id).await.expect("DB error checking generated ID");
        assert!(user_from_db.is_none(), "Generated ID was found in DB, but should be unique");
    }
    ...
    async fn test_get_all_users() {
        ...
        setup_test_db().await.expect("Failed to set up test database");

        // First, get all existing users and delete them to ensure a clean state
        let existing_users = UserStore::get_all_users().await.expect("Failed to get existing users");
        for user in existing_users {
            ...
            // Now delete the user
            UserStore::delete_user(&user.id).await.expect("Failed to delete existing user");
        }

        // Verify the database is empty
        let empty_check = UserStore::get_all_users().await.expect("Failed to check for empty database");
        ...
        UserStore::upsert_user(user1.clone()).await.expect("Failed to insert user1");
        UserStore::upsert_user(user2.clone()).await.expect("Failed to insert user2");
        UserStore::upsert_user(user3.clone()).await.expect("Failed to insert user3");

        // Call get_all_users
        let all_users = UserStore::get_all_users().await.expect("Failed to get all users");
    ...
    async fn test_get_user() {
        ...
        UserStore::upsert_user(test_user.clone()).await.expect("Failed to insert test user");

        // Test getting an existing user
        let result = UserStore::get_user("test-user-id").await.expect("Failed to get user");
        ...
        let non_existent_result = UserStore::get_user("non-existent-id").await.expect("Failed to query non-existent user");
        assert!(non_existent_result.is_none(), "Expected not to find a non-existent user");
    }
    ...
    async fn test_delete_user() {
        ...
        assert!(user_after_delete.is_none(), "User still exists after deletion");
        
        // Test deleting a non-existent user (should not error)
        let result = UserStore::delete_user("non-existent-id").await;
        assert!(result.is_ok(), "Deleting a non-existent user should not error");
    }
    ...
    async fn test_gen_new_user_id_max_retries() {
        ...
        UserStore::upsert_user(test_user1).await.expect("Failed to insert test user 1");
        UserStore::upsert_user(test_user2).await.expect("Failed to insert test user 2");
        UserStore::upsert_user(test_user3).await.expect("Failed to insert test user 3");
        
        // Test the failure case (all 3 UUIDs exist)
        {
            // Mock implementation with 3 colliding IDs
            let result = gen_new_user_id_with_mock(&["fixed-uuid-1", "fixed-uuid-2", "fixed-uuid-3"]).await;
            
            // Verify it fails with the expected error
            assert!(result.is_err());
            if let Err(CoordinationError::Coordination(msg)) = result {
                assert_eq!(msg, "Failed to generate a unique user ID after multiple attempts");
            } else {
        ...
        // Test the success case (third UUID is unique)
        {
            ...
            let result = gen_new_user_id_with_mock(&["fixed-uuid-1", "fixed-uuid-2", "fixed-uuid-4"]).await;

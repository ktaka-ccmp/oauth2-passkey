...
use crate::passkey::types::{CredentialSearchField, PasskeyCredential, PublicKeyCredentialUserEntity};
...
mod tests {
    ...
    async fn test_store_and_get_credential() {
        ...
        // Create user first
        let _ = create_test_user(user_id).await.expect("Failed to create test user");
        
        let credential = create_test_credential(credential_id, user_id, user_handle);

        // Store credential
        let store_result = PasskeyStore::store_credential(credential_id.to_string(), credential.clone()).await;
        ...
        assert!(get_result.is_ok(), "Failed to get credential: {:?}", get_result.err());
        ...
        // Cleanup
        let _ = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId(credential_id.to_string())).await;
    ...
    async fn test_get_nonexistent_credential() {
        ...
        assert!(result.is_ok(), "Getting nonexistent credential should not error");
        assert!(result.unwrap().is_none(), "Nonexistent credential should return None");
    }
    ...
    async fn test_get_credentials_by_user_id() {
        ...
        let result = PasskeyStore::get_credentials_by(CredentialSearchField::UserId(user_id.to_string())).await;
        assert!(result.is_ok(), "Failed to get credentials by user ID: {:?}", result.err());

        let credentials = result.unwrap();
        assert_eq!(credentials.len(), 2, "Should find exactly 2 credentials for user");
        
        let credential_ids: Vec<String> = credentials.iter().map(|c| c.credential_id.clone()).collect();
        assert!(credential_ids.contains(&"cred_001".to_string()));
        assert!(credential_ids.contains(&"cred_002".to_string()));

        // Cleanup
        let _ = PasskeyStore::delete_credential_by(CredentialSearchField::UserId(user_id.to_string())).await;
        let _ = UserStore::delete_user(user_id).await;
    }
    ...
    async fn test_get_credentials_by_user_handle() {
        ...
        // Store credential
        let _ = PasskeyStore::store_credential("cred_handle_test".to_string(), credential.clone()).await;

        // Get by user handle
        let result = PasskeyStore::get_credentials_by(CredentialSearchField::UserHandle(user_handle.to_string())).await;
        assert!(result.is_ok(), "Failed to get credentials by user handle: {:?}", result.err());

        let credentials = result.unwrap();
        assert_eq!(credentials.len(), 1, "Should find exactly 1 credential for user handle");
        assert_eq!(credentials[0].user.user_handle, user_handle);

        // Cleanup
        let _ = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId("cred_handle_test".to_string())).await;
        let _ = UserStore::delete_user(user_id).await;
    }
    ...
    async fn test_get_credentials_by_username() {
        ...
        let mut credential = create_test_credential("cred_username_test", user_id, "handle_username");
        ...
        // Store credential
        let _ = PasskeyStore::store_credential("cred_username_test".to_string(), credential.clone()).await;

        // Get by username
        let result = PasskeyStore::get_credentials_by(CredentialSearchField::UserName(username.to_string())).await;
        assert!(result.is_ok(), "Failed to get credentials by username: {:?}", result.err());

        let credentials = result.unwrap();
        assert_eq!(credentials.len(), 1, "Should find exactly 1 credential for username");
        assert_eq!(credentials[0].user.name, username);

        // Cleanup
        let _ = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId("cred_username_test".to_string())).await;
        let _ = UserStore::delete_user(user_id).await;
    }
    ...
    async fn test_update_credential_counter() {
        ...
        // Update counter
        let new_counter = 42;
        let update_result = PasskeyStore::update_credential_counter(credential_id, new_counter).await;
        assert!(update_result.is_ok(), "Failed to update counter: {:?}", update_result.err());

        // Verify counter was updated
        let get_result = PasskeyStore::get_credential(credential_id).await.unwrap().unwrap();
        assert_eq!(get_result.counter, new_counter, "Counter should be updated to new value");
    ...
    async fn test_update_credential_user_details() {
        ...
        let update_result = PasskeyStore::update_credential(credential_id, new_name, new_display_name).await;
        assert!(update_result.is_ok(), "Failed to update credential: {:?}", update_result.err());
    ...
    async fn test_update_credential_last_used_at() {
        ...
        let update_result = PasskeyStore::update_credential_last_used_at(credential_id, new_last_used).await;
        assert!(update_result.is_ok(), "Failed to update last_used_at: {:?}", update_result.err());

        // Verify timestamp was updated
        let get_result = PasskeyStore::get_credential(credential_id).await.unwrap().unwrap();
        assert_ne!(get_result.last_used_at, original_last_used, "last_used_at should be different");
        // Allow some tolerance for timestamp comparison (within 1 second)
        let diff = (get_result.last_used_at - new_last_used).num_milliseconds().abs();
    ...
    async fn test_delete_credential_by_credential_id() {
        ...
        assert!(get_result.is_some(), "Credential should exist before deletion");

        // Delete credential
        let delete_result = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId(credential_id.to_string())).await;
        assert!(delete_result.is_ok(), "Failed to delete credential: {:?}", delete_result.err());

        // Verify it's gone
        let get_result_after = PasskeyStore::get_credential(credential_id).await.unwrap();
        assert!(get_result_after.is_none(), "Credential should not exist after deletion");
    ...
    async fn test_delete_credentials_by_user_id() {
        ...
        // Verify they exist
        let get_result = PasskeyStore::get_credentials_by(CredentialSearchField::UserId(user_id.to_string())).await.unwrap();
        assert_eq!(get_result.len(), 2, "Should have 2 credentials before deletion");

        // Delete all credentials for user
        let delete_result = PasskeyStore::delete_credential_by(CredentialSearchField::UserId(user_id.to_string())).await;
        assert!(delete_result.is_ok(), "Failed to delete credentials by user ID: {:?}", delete_result.err());

        // Verify they're gone
        let get_result_after = PasskeyStore::get_credentials_by(CredentialSearchField::UserId(user_id.to_string())).await.unwrap();
        assert_eq!(get_result_after.len(), 0, "Should have 0 credentials after deletion");
    ...
    async fn test_credential_isolation() {
        ...
        // Create users first
        let _ = create_test_user(user_id_1).await.expect("Failed to create test user 1");
        let _ = create_test_user(user_id_2).await.expect("Failed to create test user 2");
        ...
        let creds_user1 = PasskeyStore::get_credentials_by(CredentialSearchField::UserId(user_id_1.to_string())).await.unwrap();
        let creds_user2 = PasskeyStore::get_credentials_by(CredentialSearchField::UserId(user_id_2.to_string())).await.unwrap();

        // Verify isolation
        assert_eq!(creds_user1.len(), 1, "User 1 should have exactly 1 credential");
        assert_eq!(creds_user2.len(), 1, "User 2 should have exactly 1 credential");
        assert_eq!(creds_user1[0].credential_id, "cred_iso_001");
        assert_eq!(creds_user2[0].credential_id, "cred_iso_002");

        // Cleanup
        let _ = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId("cred_iso_001".to_string())).await;
        let _ = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId("cred_iso_002".to_string())).await;
    ...
    async fn test_concurrent_operations() {
        ...
        let handles = (0..5).map(|i| {
            ...
            tokio::spawn(async move {
                ...
                // Store credential
                let store_result = PasskeyStore::store_credential(credential_id.clone(), credential).await;
                assert!(store_result.is_ok(), "Concurrent store should succeed");
                
                // Get credential
                let get_result = PasskeyStore::get_credential(&credential_id).await;
                assert!(get_result.is_ok() && get_result.unwrap().is_some(), "Concurrent get should succeed");
                
                // Clean up
                let _ = PasskeyStore::delete_credential_by(CredentialSearchField::CredentialId(credential_id)).await;
                let _ = UserStore::delete_user(&user_id).await;
            })
        }).collect::<Vec<_>>();

...
async fn test_prepare_logout_response_success() {
    ...
    headers.insert(
        "cookie",
        HeaderValue::from_str(&format!("{}={}", SESSION_COOKIE_NAME.as_str(), session_id)).unwrap(),
    );
...
async fn test_get_user_from_session_requires_database() {
    use crate::session::main::test_utils::{cleanup_test_resources, create_test_user_and_session};

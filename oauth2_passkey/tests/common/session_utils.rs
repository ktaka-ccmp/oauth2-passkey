...
/// Verify that registration was successful with valid session and user info
pub async fn verify_successful_registration(
    browser: &MockBrowser,
    expected_test_user: &crate::common::fixtures::TestUser,
    auth_type: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        browser.has_active_session().await,
        "Session should be established after successful {auth_type} registration"
    );

    let user_info = browser.get_user_info().await?;
    assert!(
        user_info.is_some(),
        "User info should be available after successful {auth_type} registration"
    );
    ...
    assert_eq!(
        label,
        Some(expected_test_user.name.as_str()),
